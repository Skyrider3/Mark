    ...
    const fetchStockData = async () => {
      try {
        ...
        if (stockResult[0]) {
          ...
          setStockData({ companyName: symbol, symbol: symbol });
          console.log("no data available for this stock");
        }

        // fetch news data
        const newsResponse = await fetch(
          `https://financialmodelingprep.com/api/v3/stock_news?tickers=${symbol}&limit=5&apikey=${API_KEY}`
        );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="bg-white shadow rounded-lg p-4">
            ...
            <p>
              ...
              {stockData.volAvg?.toLocaleString() || "N/A"}
            </p>
            <p>
              <strong>Market Cap:</strong> $
              {(stockData.mktCap / 1000000000)?.toFixed(2) || "N/A"} B
            </p>
            <p>
              <strong>52 Week High:</strong> $
              {stockData.range?.split("-")[1].trim() || "N/A"}
            </p>
            <p>
              <strong>52 Week Low:</strong> $
              {stockData.range?.split("-")[0].trim() || "N/A"}
            </p>
          </div>
          <div className="bg-white shadow rounded-lg p-4">
