...
# try:
#     client = QdrantClient("localhost", port=6333)  # Adjust host and port as needed
...
class MultiDocumentReActAgent:
    ...
    def load_from_existing_context(self):
        ...
        if not self.index_loaded:
            # load data
            ge_docs = SimpleDirectoryReader(input_files=["./data/10k/ge_ar2023_annualreport.pdf"]).load_data()
            # print(ge_docs)
            honeywell_docs = SimpleDirectoryReader(input_files=["./data/10k/Honeywell_2024.pdf"]).load_data()

            # build index
            self.ge_index = VectorStoreIndex.from_documents(documents=ge_docs, storage_context=self.ge_storage_context)
            self.honeywell_index = VectorStoreIndex.from_documents(documents=honeywell_docs, storage_context=self.honeywell_storage_context)
        ...
        self.query_engine_tools = [
            ...
            QueryEngineTool(
                ...
                metadata=ToolMetadata(
                    name="honeywell_10k",
                    description=(
                        "Provides detailed financial information about Honeywell for the year 2024. "
                        "Input a specific plain text financial query for the tool"
                    ),
                ),
            ),
